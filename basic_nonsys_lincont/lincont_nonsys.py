...
if __name__ == '__main__':
    ...
    pend = CartPole(num = 1, alpha = 0.001, radius = 0.005, t_bound=30)
    pend.setLinearControl(Kp, gain, x0, target = np.array([0, 0.2, 0, 0]))
    ...
    for i in range(num):
        var['pendulum' + str(i) + 'Kp'] = list(pends[i].Kp)
        var['pendulum' + str(i) + 'gain'] = list(pends[i].gain)
    ...
    for j in range(epochs):
        ...
        K_sets = []
        for i in range(num):
            err.append(pends[i].runEpoch(10))
            K_sets.append(pends[i].Kp)
